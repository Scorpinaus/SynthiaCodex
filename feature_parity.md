...
| Git status and file diff | Working/staged views, changed-file selection, refresh, repository selection across attached folders/worktrees, old/new line-numbered unified-diff rows, latest-review annotations with unmatched fallback, pending user-comment cards, and accessible hunk operations | **Near** | Commit/Branch/Last turn diff loading remains absent. |
| Stage, unstage, discard, commit | Repository-scoped whole-file actions plus stage/confirmed-discard for eligible Working hunks, unstage for eligible Staged hunks, and commit message UI | **Full** | Partial operations deliberately apply only to ordinary tracked text modifications; file-level metadata and binary changes retain safe whole-file actions. |
...
2. **Interactive Git review (hunk operations, structured findings, and user comments implemented):** add Commit/Branch/Last turn diff loading on top of the implemented multi-repository selector, dedicated review targets, typed inline reviewer annotations, user-authored old/new-side comments, and stage/unstage/confirmed-discard for tracked text hunks.
