...
| `SynthiaCode.Presentation` | Pure Markdown document model and syntax parser | None |
...
flowchart LR
    ...
    App --> Presentation["SynthiaCode.Presentation"]
...
The architecture is layered but deliberately pragmatic rather than a strict clean-architecture implementation. Core and Presentation have no project dependencies. Application contains the portable harness runtime and the complete conversation feature slice. Infrastructure contains operating-system and process adapters. Provider translation lives in a harness project. App owns WPF, Markdown destination policy and rendering, and Codex-specific side features until portable contracts exist for them.
...
`MainViewModel` is the shell coordinator. `ProjectThreadViewModel`, `TaskViewModel`, `TerminalViewModel`, `DiagnosticsViewModel`, `AccountViewModel`, `GitViewModel`, `SkillsViewModel`, and `EffectiveCodexSettingsViewModel` own feature presentation state and commands. `TaskViewModel` is a compatibility facade over conversation, composer, agents, goals, and options feature models. `GitViewModel` composes repository selection, changes and diff, Git actions, and push-planning models. The shell supplies explicit operation delegates or immutable-on-read context callbacks and receives status/selection callbacks; feature view models do not reference or control one another.
...
`TaskView` is a small composition shell. `TaskConversationView`, `TaskComposerView`, `TaskAgentsView`, `TaskGoalsView`, and `TaskOptionsView` own their XAML namescopes and feature event handlers. Each control binds through its matching feature model. The root task and Git view models keep compatibility properties for application callers and established tests while new XAML uses the focused components. Timeline, raw-event, thread, diagnostic, recent-project, and changed-file lists use recycling virtualization and content scrolling. The transcript keeps pixel scrolling for variable-height turns and uses a dedicated 72 px near-latest coordinator that pauses following after deliberate upward navigation and resets follow state when chat identity changes; terminal output remains coalesced through the existing 50 ms presentation path.

The pure `SynthiaCode.Presentation.Markdown` parser produces a WPF-free document model for headings, prose, lists, definitions, tables, quotes, code, links, images, footnotes, and allowed HTML shapes. `MarkdownTextBlock` converts that model to WPF inlines and controls. `ExternalUriPolicy` and `LocalImageResourcePolicy` remain App-side destination policies, so parsing does not open links, read files, or create WPF objects.
...
`GitRepositorySelectionViewModel` projects the repositories found in the active worktree and attached project folders. `GitChangesViewModel` projects scopes, changed files, selected diff rows, findings, and inline comments. `GitActionsViewModel` exposes refresh, file and hunk mutations, comment actions, commit, push, and repository deep links. `GitPushPlanningViewModel` owns the displayed-root and named-branch preconditions used before and after confirmation. `GitViewModel` remains their compatibility and orchestration facade. Last turn intentionally switches the header to **All repos**. Every Git mutation and Editor/Explorer deep link uses a selected repository root. File/hunk mutations remain available only in Unstaged or Staged, commit requires staged changes, and push requires the selected repository to retain a named branch through confirmation and execution.
...
| xUnit suite | 390 passing tests | Includes the 307-case legacy behavioral matrix plus focused architecture, migration, protocol, integration, and presentation tests discovered individually by xUnit. |
