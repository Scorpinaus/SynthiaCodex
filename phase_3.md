...
- `scripts\publish-portable.cmd` produced the self-contained `win-x64` artifact in `portable\NativeCodexAssistant\`. NuGet vulnerability-feed checks emitted `NU1900` because the service index was unreachable, but publish completed and the executable was verified by the script.
