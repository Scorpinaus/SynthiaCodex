...
| Phase 8 - Hunk-operation research and red contract | Rechecked the official review-pane stage/revert workflow, recorded the bounded implementation plan and safety boundary, and added four focused parser, real-Git, view-model, confirmation, and accessibility cases. The pre-implementation build failed at the intended missing hunk types and service contract. | **Complete** |
| Phase 9 - Hunk-level Git operations | Added immutable single-hunk patch extraction, parser verification and an 8 MiB input bound, standard-input `git apply` for stage/unstage/discard, view-specific accessible `@@` actions, confirmed destructive discard, repository refresh, and a deliberate tracked-text-modification eligibility boundary. | **Complete** |
| Phase 10 - Hunk verification | All four hunk behavioral cases and all 47 Git-filtered tests pass. The complete Debug and Release suites are 314/320 with exactly the same six protected-baseline failures and no new regression; one known fork timeout on the first Release run passed 37/37 in isolation and the full rerun matched baseline. Debug and Release solution rebuilds complete with zero warnings and errors. | **Complete** |
...
| Conversation lifecycle protocol | Start/resume/read/fork/archive/unarchive/name/goal and rollback are typed; completed-response forks use an inclusive `lastTurnId` boundary directly | **Near** — `thread/delete`, `thread/metadata/update`, `thread/compact/start`, and `thread/unsubscribe` are unwired. Prompt editing still depends on deprecated `thread/rollback`. |
...
| Git and worktree lifecycle | **Moderate** | Core isolation, file- and hunk-level Git operations, all five diff scopes, dedicated review targets, inline reviewer findings, user comments, and confirmed native branch push exist; handoff, PR, snapshots, and setup actions do not. |
...
| Resume, fork, archive, unarchive | Typed app-server lifecycle flows and UI actions; completed-response forks send the selected server turn as the inclusive `lastTurnId` boundary while full-thread forks omit it | **Full** | None material for these lifecycle actions; prompt-edit rollback migration is tracked separately above. |
...
| Unfamiliar-project trust | First-open trust gate covers folder browsing, recent projects, and startup restoration; trusted and untrusted decisions persist in Codex configuration | **Full** | None material. |
...
| Push and pull request | Native branch push confirms the displayed repository branch and remote, uses its existing upstream, or sets the sole configured remote as upstream | **Partial** | GitHub pull-request creation/status remains missing; branches without an upstream and with multiple remotes require explicit upstream configuration. |
...
5. **Thread lifecycle parity remains Near.** Completed-response forks now use the checked-in inclusive `lastTurnId` contract directly. Permanent `thread/delete`, server-side pin metadata, manual compaction, and unsubscribe remain unwired, and prompt editing still depends on deprecated `thread/rollback`.
...
1. **Current lifecycle contract (direct response forks implemented):** define a migration away from prompt editing's deprecated `thread/rollback`, and wire permanent delete, server pin metadata, and manual compaction against the checked-in Codex `0.147.0` contracts.
...
5. **Pull requests (branch push implemented):** add GitHub PR creation/status.
