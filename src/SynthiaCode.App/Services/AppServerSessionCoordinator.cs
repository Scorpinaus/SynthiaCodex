...
public sealed class AppServerSessionCoordinator : IAppServerSessionCoordinator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await discoveryClient
            .ListPermissionProfilesAsync(cwd, cancellationToken)
            .ConfigureAwait(false);
        if (!ReferenceEquals(discoveryClient, client))
        {
            throw new InvalidOperationException("Permission profiles were returned by a stale app-server connection.");
        }
